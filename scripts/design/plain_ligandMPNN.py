...
parser.add_argument("--design_positions", nargs="+", type=int, help="Residue positions to design.")
...
# pyr.init(f"{extra_res_fa} -dalphaball {DAB} -run:preserve_header -mute all -gen_potential "
#          f"-multithreading true -multithreading:total_threads {NPROC} -multithreading:interaction_graph_threads {NPROC}")
...
for N in range(args.nstruct):
    ...
    while len(filt_scores) == 0:
        ...
        print("Identifying positions to design")
        ...
        not_design = [res.seqpos() for res in _pose2.residues if res.seqpos() not in args.design_positions and not res.is_ligand()]
        
        fixed_residues = []
        for rn in list(set(not_design)):
            fixed_residues.append(_pose2.pdb_info().chain(rn)+str(_pose2.pdb_info().number(rn)))
            
        print(f"Fixed residues: {fixed_residues}")

        # Setting up MPNN runnerW
